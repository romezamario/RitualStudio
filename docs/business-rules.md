...
- La home (`/`) funciona como una composición editorial por secciones (hero split, mensaje central, grid de líneas, banner emocional, carrusel y bloque de confianza) con accesos directos a: tienda (`/marketplace`), cursos (`/cursos`), diseño a medida (`/custom`), eventos (`/eventos`) y contacto (`/contacto`).
