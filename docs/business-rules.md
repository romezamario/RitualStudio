...
- En `/marketplace`, la card completa de cada producto funciona como acceso al detalle (`/marketplace/[slug]`); no se muestra un botón independiente de **Ver detalle** para productos, y las acciones de carrito/compra permanecen separadas.
- Los productos pueden marcarse como `is_test_product`; estos solo son visibles en catálogo/detalle para usuarios con rol `admin` y se ocultan para usuarios normales (incluyendo modo producción y test).
...
- En el detalle de producto (`/marketplace/[slug]`), la ficha visible prioriza precio, tamaño y **Flores que habitan esta pieza**; las secciones “Ideal para” y “Entrega” no se muestran en el bloque informativo principal.
- En administración de productos (`/admin/productos`), tamaño y lista de flores son campos editables obligatorios para mantener la ficha comercial del PDP.
...
- La pantalla `/checkout/exito` muestra el comprobante canónico con productos y la dirección de entrega registrada en `orders.metadata.delivery_address`.
